...
const isDomainVerificationError = (error: unknown): boolean => {
  const message = JSON.stringify(error ?? {}).toLowerCase();
  return message.includes("domain is not verified") || message.includes("domain nicht verifiziert");
};
...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    let emailResponse = await resend.emails.send({
      from: "Coach Anfrage <jona@fels-coach.de>",
...
<body style="margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; background-color: #f5f5f5;">
  <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background-color: #f5f5f5; padding: 40px 20px;">
    ...
    if (emailResponse.error && isDomainVerificationError(emailResponse.error)) {
      console.warn("Primary Resend sender not verified yet; retrying contact notification with Resend test sender.");
      emailResponse = await resend.emails.send({
        from: "Coach Anfrage <onboarding@resend.dev>",
        to: ["jona@fels-coach.de"],
        reply_to: trimmedEmail,
        subject: `Neue Coaching-Anfrage von ${trimmedName}`,
        html: `
