...
export const translations: Translations = {
  ...
  "privacy.s6.p2": { de: "Die Verarbeitung läuft über die Plattform Lovable (Lovable AB, Schweden): Eine serverseitige Funktion (Edge Function) nimmt deine Anfrage entgegen, speichert sie in einer durch Row-Level-Security geschützten Datenbanktabelle (Supabase, Inc., USA – Tabelle „contacts“, Lese­zugriff ausschließlich für mich als Admin) und löst den Versand einer Benachrichtigungs-E-Mail an mich über den Dienst Resend, Inc. (USA) aus. Zum Schutz vor Missbrauch wird zusätzlich deine IP-Adresse kurzfristig in einer Rate-Limit-Tabelle gespeichert (max. 5 Anfragen pro IP / 60 Minuten). Bei der Nutzung von Telegram weise ich darauf hin, dass Daten auf den Servern des Anbieters (ggf. außerhalb der EU) verarbeitet werden.", en: "Processing runs via the Lovable platform (Lovable AB, Sweden): a server-side function (Edge Function) receives your request, stores it in a database table protected by Row-Level Security (Supabase, Inc., USA – table “contacts”, read access restricted to me as admin) and triggers a notification email to me via the service Resend, Inc. (USA). To prevent abuse, your IP address is also briefly stored in a rate-limit table (max. 5 requests per IP / 60 minutes). When using Telegram, please note that data may be processed on the provider's servers (potentially outside the EU)." },
